...
import { BarChart3, Users, Swords, Settings2 } from "lucide-react";
...
export default async function TeamAdminLayout({
  ...
  const stageLabel =
    ev.stage === "setup" ? "Thiết lập" :
    ev.stage === "group" ? "Vòng bảng" :
    ev.stage === "knockout" ? "Knockout" : "Hoàn thành";

  const nav = [
    { href: `/team/${slug}`, label: "Tổng quan", short: "Tổng quan", icon: BarChart3 },
    { href: `/team/${slug}/squads`, label: "Đội & VĐV", short: "Đội & VĐV", icon: Users },
    { href: `/team/${slug}/ties`, label: "Đối đầu", short: "Đối đầu", icon: Swords },
    { href: `/team/${slug}/settings`, label: "Cài đặt", short: "Cài đặt", icon: Settings2 },
  ];
      ...
      <div className="border-b bg-gradient-to-r from-primary/10 via-secondary/30 to-primary/5">
        <div className="mx-auto flex w-full max-w-7xl items-center justify-between gap-3 px-3 py-2.5 sm:px-4 sm:py-3">
          <div className="min-w-0">
            ...
            <p className="text-xs text-muted-foreground">Giải đồng đội · {stageLabel}</p>
          </div>
